...
import { updateTeacherProfile, updateTeacherPassword } from './actions';
...
export function SettingsForm({ profile, userEmail }: { profile: Profile; userEmail: string }) {
  ...
  function handleProfile(e: React.FormEvent<HTMLFormElement>) {
    ...
    startProfile(async () => {
      try { await updateTeacherProfile(fd); setProfileMsg('Changes saved.'); }
  ...
  function handlePassword(e: React.FormEvent<HTMLFormElement>) {
    ...
    startPassword(async () => {
      try { await updateTeacherPassword(fd); setPasswordMsg('Password updated.'); (e.target as HTMLFormElement).reset(); }
